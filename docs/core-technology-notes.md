...
- **declare_dag — the 7th butler tool (2026-09)**: the model can declare a BATCH graph mid-turn (the dynamic face's batch form, §1 pillar #1). `ToolCtx.declareDag` rides the same DagRunner as `POST /v1/dag` — with `todoSessionId = the declaring session`, so node progress projects into the declarer's todo list. Every submission is audited (`appendAudit`) like every other butler action. The spec travels as `unknown` through core (no DAGSpec import into the tool layer) and is validated at the runtime boundary.
...
- **Skill import through the directory seam (2026-09)**: `POST /v1/skills {name, description?, body}` writes `pluginsDir/skills/<name>/SKILL.md` (frontmatter description + body) — directory-as-registration means the catalog (`GET /v1/skills` re-discovers per request) and the `skill` tool pick it up with no restart. The slug regex rejects traversal shapes; the write is the ONLY mutation path the transport adds to the plugins dir (deletion stays a file-system/host concern).
...
- **Microcompact (projection-layer, 2026-09)**: `clearStaleToolResults` blanks tool outputs older than `keepRecent` when the visible projection crosses a char threshold — cheap, provider-agnostic pressure relief that never touches the log (the result stays durable; only the model-visible projection shrinks). Borrowed from ZCode's `LocalToolResultClear`; ours is a pure projection function run at read time, not an event, so "model-visible ⟺ logged" is preserved (the log is the superset).
...
- **File-layer readback completeness (fix, 2026-09)**: `loadRuntimeSettings` now reads `channels`/`mcpServers` back from the agent-home config file — previously PUT persisted them but the loader never read them, so GET always showed "未配置" and MCP servers never mounted on restart. Rule of thumb: every `AgentHomeConfig` field needs BOTH a write path (`writeAgentHomeConfig` merge) and a read path (`loadRuntimeSettings`).

## 29. Global event bus — one SSE feed for every session (2026-09)

List-driven UIs were polling (`/v1/sessions` every 4s, per-tab intervals, approvals by remount). The bus replaces that with ONE pushed feed, modeled on opencode's `GET /api/event`:

- **Server** (`server.ts`): `GET /v1/events/stream` — SSE, frames `data: {sessionId, event}` (LoopEvent frames exactly as the per-session stream, plus the prompt-stream terminal markers). Every `attach(id, app)` (both `apps.set` sites: create + lazy re-attach + host resolver) plants a PERMANENT per-app relay that fans `app.onEvent` into the global listener set; `detach` (session delete) unsubscribes it, so relays can't leak across a session's lifetime. Listener errors are isolated per listener — a broken SSE consumer never sinks a turn (same rule as `app.onEvent` itself).
- **Terminal-frame forwarding**: a turn that fails at the LLM call throws WITHOUT a LoopEvent (only the prompt stream's catch would know) — `promptStream` forwards its `result`/`error` terminal frames onto the bus, so bus consumers see the settle either way. The bus is LoopEvent-shaped, not log-shaped: durable state still folds from `GET /v1/events`; the bus only says "something moved".
- **Client** (`apps/web/src/api/bus.ts`, host-side): one fetch-ReadableStream pump with opencode's discipline — fixed 250 ms reconnect, a generation counter kills stale loops, an outage logs once (not per retry), and subscribers are a plain Set with isolated failures. `useBusRefresh(match, cb, throttleMs)` is the consumption pattern: components refetch their slice on settle frames (`result`/`done`/`error`) instead of interval-polling.
- **Scope honesty**: the bus forwards LOCALLY-attached sessions only (remote-owned sessions have no live App here — same scope as the per-session stream); it does not carry stored events, approvals lifecycle, or DAG state — those stay pull-based (approvals additionally refresh on `tool` frames, when a gate is most likely raised).

## 30. baseUrl normalization — the trailing-/v1 trap (2026-09)

Provider docs hand out baseUrls in both spellings (`https://api.minimaxi.com/anthropic` vs an OpenAI-style `…/v1`), and every protocol path in the adapter already starts with `/v1` (`/v1/chat/completions`, `/v1/responses`, `/v1/messages`). Concatenating blindly produced `/v1/v1/…` — the chat path silently worked only for users who stripped it themselves, and `listModels` (the settings page's "拉取模型") 404'd into an empty list for everyone else.
