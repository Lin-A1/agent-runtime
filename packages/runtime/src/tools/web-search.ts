 ...
 * key. It scrapes the public HTML endpoints of DuckDuckGo (html + lite) and
 * falls back to Bing's public results page, first non-empty result set wins.
 * Read-only (sideEffects: false) and gated by the host's `enableWeb` switch
 * like web_fetch, because it escapes the fs sandbox the same way.
 *
...
type EngineName = "duckduckgo-html" | "duckduckgo-lite" | "bing"
  ...
  return {
    ...
    execute: async (input: unknown, ctx) => {
      ...
      for (const engine of ENGINES) {
        ...
        return { results: results.slice(0, limit), engine: engine.name }
      }
 ...
 *  the first `<p>` in the block as the snippet. */
