 ...
 */
import type { ChatImage, FileContent, ModelCallRow, PanelInfo, SessionRow, StoredEventRow, TodoItem } from "./types"
...
export function foldTranscript(events: StoredEventRow[]): TranscriptItem[] {
  ...
  // A steer note renders only when this prompt actually interrupted a RUNNING
  // turn: the engine admits every prompt() through the inbox's steer lane, so
  // the delivery label alone marks every turn. Ordering is the honest signal —
  // a genuine steer's Prompted lands before the running turn's terminal
  // StepEnded (finish != "tool") or Interrupted.
  let turnClosed = true
  ...
  for (const e of events) {
    ...
    if (e.type === "Session.StepEnded") {
      if (String(d.finish ?? "tool") !== "tool") turnClosed = true
      continue
    }
    ...
    if (e.type === "Session.Spawned") {
      pushNote(`已派生子会话${d.via ? `（${String(d.via)}）` : ""}`, "info")
      continue
    }
 ...
 *  per-turn panels from the fold, this flattens the whole log). */
export function foldPanels(events: StoredEventRow[]): PanelInfo[] {
  return events
    .filter((e) => e.type === "Session.PanelPosted")
    .map((e) => ({
      panelId: String(e.data?.panelId ?? e.seq),
      kind: String(e.data?.kind ?? "markdown"),
      ...(e.data?.title ? { title: String(e.data.title) } : {}),
