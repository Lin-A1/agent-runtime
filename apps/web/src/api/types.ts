...
export interface SessionRow {
  ...
  model?: string
  parentId?: string
  /** How this session came to be: a DAG node child or a spawn_agent child
   *  (registry fold of Session.Spawned). The tree nests these under parentId. */
  origin?: "dag" | "spawn"
...
export interface Schedule {
  ...
  lastResult?: "ok" | "error"
  lastError?: string
  /** Next scheduled fire (epoch ms) — present when the schedule is enabled. */
  nextFireAt?: number
}
...
export interface DagNodeStatus {
  ...
  dependsOn?: string[]
  /** Registered child session driving this node — click-through target. */
...
export interface TodoItem {
  ...
  /** Present-tense label shown while in_progress (todo_write activeForm). */
  activeForm?: string
}
 ...
 *  / the `panel` LoopEvent). `kind` selects the renderer: diff / markdown /
 *  table / image / url / form (open set). */
